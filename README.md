...
| 目录 | 用途 |
|------|------|
| `v1.0/` | 当前**确认可执行**的版本（博客本体）。后续迭代用 `v1.1/`、`v2.0/`…，不删除旧版本。 |
| `v1.0/tmp/` | 该版本的中间产物（草稿、截图、临时导出等），不进最终交付。 |
...
cd v1.0
npm ci
npm run build      # 产物在 v1.0/dist
```

推送 `main` 后由根目录 `.github/workflows/deploy.yml` 自动构建并部署（工作目录 `v1.0`）。
